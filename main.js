 ...
 */
      ...
      set = function() {
        ...
        var symbol = turn === "X" ? "🖤" : "⭕️"; /* Changed symbols using awesome font api link this makes it more fun */
        $(this).html(symbol);
        moves += 1;
        score[turn] += $(this)[0].indicator;
        if (win(score[turn])) {
          var winner = turn === "X" ? "Black Heart" : "Circle";
          showAlert("Congratulations, " + winner + " wins! 🎉 Let's play again!");
          startNewGame();
        } else if (moves === SIZE * SIZE) {
          showAlert("It's a tie! 🤝 Let's have a rematch!");
          startNewGame();
        } else {
          turn = turn === "X" ? "O" : "X";
        }
